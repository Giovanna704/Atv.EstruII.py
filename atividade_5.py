...
class ArvoreAVL:
    ...
    def _inserir_recursivo(self, no_atual, chave):
        ...
        if chave < no_atual.chave:
            ...
            # Não permitimos duplicatas
            raise Exception(f"Chave duplicada detectada: {chave}")
    ...
    def encontrar_nos_intervalo(self, chave1, chave2):
        ...
        def _em_ordem_intervalo(no):
            if no is None:
                return
            # Se a chave atual pode ter valores >= chave1, visita esquerda
            if chave1 < no.chave:
                _em_ordem_intervalo(no.esquerda)
            # Se a chave está dentro do intervalo, adiciona
            if chave1 <= no.chave <= chave2:
                resultado.append(no.chave)
            # Se a chave atual pode ter valores <= chave2, visita direita
            if chave2 > no.chave:
                _em_ordem_intervalo(no.direita)
    ...
    # Métodos auxiliares para testes (percursos)
    def percurso_em_ordem(self):
        """Retorna lista com chaves em ordem (usado para ver a estrutura)."""
        res = []
        def _em_ordem(no):
            if no is None:
                return
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Percurso em-ordem após inserções:", arvore_avl.percurso_em_ordem())
